...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Sat Sep 12 18:24:54 CEST 2020
  */
...
class CUP$parser$actions {
	...
	//	Lista con las cuadruplas de las cadenas de caracteres
    ...
    throws java.lang.Exception
    {
      ...
      switch (CUP$parser$act_num)
		...
		Token string = (Token)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
						...
						//printamos el c骴igo intermedio generado
		...
		Expresion e = (Expresion)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
						...
						//	Se a馻de la expresion a la lista de parametros
						p.addE(e);
		...
		ParFuncion pF = (ParFuncion)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
						...
						if(symbol.getcP().getpP().getpLP().getlistPP()!=null) {
							for(int i=0;i<symbol.getcP().getpP().getpLP().getlistPP().size();i++) {
								...
								TipoVar tipoVar   = procParam.gettV();
								for(int j=0;j<cadIdVar.getListId().size();j++){
								//	Se comprueba si son iguales los tipos
									TypeIF tipoF1 = null;
									List<ScopeIF> scopes = scopeManager.getAllScopes();
									for(ScopeIF scopeFor : scopes){
										TypeTableIF typeTableFor = scopeFor.getTypeTable();
		...
		SentElse sE = (SentElse)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
						...
						IntermediateCodeBuilder cb= new IntermediateCodeBuilder(scope);
						cb.addQuadruples(e.getIntermediateCode()); //a馻dir el c骴igo de la expresi髇
						cb.addQuadruple("BRF", expTemp, etiquetaElse); //si no se cumple la condicion salto etiquetaElse
						cb.addQuadruples (s.getIntermediateCode()); // a馻dir c骴igo sentencias parte IF
						cb.addQuadruple("BR", etiquetaFin); //salto a la etiquetaFin
						cb.addQuadruple("INL", etiquetaElse); // insertar etiquetaElse
						cb.addQuadruples(sE.getIntermediateCode()); //a馻dir c骴igo sentencias parte Else
		...
		Sentencia s = (Sentencia)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
						...
						//	Se a馻de la sentencia a listSentencia
						lS.addS(s);
		...
		ProcListParam pLP = (ProcListParam)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
						...
						//	Se a馻de procParam a la lista de procListParam
						pLP.addPP(pP);
						RESULT = pLP;
		...
		TipoRetorno tR = (TipoRetorno)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
						...
						//	Se a馻de a la tabla de simbolos el identificador junto al simbolo de funcion o procedimiento
						...
						//	Se crean y a馻den los tipos simples y el tipo del procedure
						...
						//	Se a馻den los simbolos a la tabla de simbolos de todos parametros
		...
		StmSubprogram sS = (StmSubprogram)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
						...
						//	Se a馻de stmSubprogram a la lista y se itera sobre si mismo
		...
		ExpVar eV = (ExpVar)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
						...
						//	Se a馻de expVar a la lista de sentVar y se itera de nuevo
						sV.addEV(eV);
						RESULT = sV;
		...
		ExpTipo eT = (ExpTipo)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
						...
						//	Si hay varias expTipo se a馻de y se itera de nuevo
						sT.addET(eT);
						RESULT = sT;
		...
		ValorConst vC = (ValorConst)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
						...
						//	Se a馻de el identificador a la tabla de simbolos como constante
		...
		ExpConst eC = (ExpConst)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
						...
						//	A馻do la expConst a la sentConst
		...
		ExpConst eC = (ExpConst)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
						...
						//	A馻do la expConst a la lista de sentConst y se llama a si mismo de nuevo
						sC.addEC(eC);
		...
		Axiom ax = (Axiom)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
						...
						//	Se declaran las direcciones iniciales para variables globales
						int gAddress = 0;
						int gOffset = 0;
						ScopeIF currentScope = null;
						Map<String, SymbolProcedure> procedures = new HashMap<String, SymbolProcedure>();
						//	Se recorren todos los ambitos del programa
						for (ScopeIF scope : scopeManager.getAllScopes()) {        
							int lOffset = 0;
							int fOffset = 1;
							boolean scopeGlobal = false;
							//	Se recorre la tabla de simbolos y se van reservando segun su tipo
							for(SymbolIF symbol : scope.getSymbolTable().getSymbols()) {
								if(symbol instanceof SymbolVariable) {
									SymbolVariable variableSymbol = (SymbolVariable)symbol;
									if (scope.getLevel() == 0) {
										variableSymbol.setAddress(gAddress);
										gAddress += variableSymbol.getType().getSize();
									} else {						
										//	Se reserva espacio para el estado y el enlace de control
										variableSymbol.setAddress(fOffset); // Ademas de la dir de retorno se reserva espacio para el estado y el enlace de control
										fOffset += variableSymbol.getType().getSize();
						...
						//	Se recorre la tabla de temporales y se asignan posiciones
						for (TemporalIF t:temporalesAmbito) {
							if (scope.getLevel() == 0) {
								t.setAddress(lOffset);
								lOffset++;
							} else {
								t.setAddress(fOffset);
						...
						//	Se da la instruccion indicando la posicion a partir de la cual se almacena el programa y, se introducen las instrucciones del programa
						IntermediateCodeBuilder cb = new IntermediateCodeBuilder(currentScope);
						LabelFactoryIF labelFactory = new LabelFactory ();
						cb.addQuadruple("DATA", null, gAddress + gOffset, gAddress);
